...
struct Cell {
    ...
    void setWall(Direction dir, bool hasWall) {
        switch(dir) {
            case NORTH: north_wall = hasWall; break;
            case EAST: east_wall = hasWall; break;
            case SOUTH: south_wall = hasWall; break;
            case WEST: west_wall = hasWall; break;
        }
    }
};
...
public:
    ...
    void initializeBoundaryWalls() {
        ...
        for(int y = 0; y < maze_height; y++) {
            for(int x = 0; x < maze_width; x++) {
                if(x == 0) maze[y][x].setWall(WEST, true);
                if(x == maze_width - 1) maze[y][x].setWall(EAST, true);
                if(y == 0) maze[y][x].setWall(SOUTH, true);
                if(y == maze_height - 1) maze[y][x].setWall(NORTH, true);
            }
        }
    }
    ...
    Position getAdjacentPosition(Position pos, Direction dir) {
        switch(dir) {
            case NORTH: return Position(pos.x, pos.y + 1);
            case EAST: return Position(pos.x + 1, pos.y);
            case SOUTH: return Position(pos.x, pos.y - 1);
            case WEST: return Position(pos.x - 1, pos.y);
            default: return pos;
        }
    }
    ...
    void floodFill() {
        ...
        while(!q.empty()) {
            ...
            for(int dir = 0; dir < 4; dir++) {
                ...
                if(maze[current.y][current.x].hasWall(direction)) continue;
                ...
                if(maze[next.y][next.x].flood_value != -1) continue;
    ...
    Direction getBestDirection() {
        ...
        for(int dir = 0; dir < 4; dir++) {
            ...
            // Skip if there's a wall
            if(maze[current_pos.y][current_pos.x].hasWall(direction)) continue;
    ...
    std::vector<Position> getShortestPath() {
        ...
        while(!(pos == target_pos)) {
            ...
            for(int dir = 0; dir < 4; dir++) {
                ...
                if(maze[pos.y][pos.x].hasWall(direction)) continue;
    ...
    void solveMaze() {
        ...
        while(exploring && step_count < 1000) { // Safety limit
    ...
    void printMaze() {
        for(int y = maze_height - 1; y >= 0; y--) {
            ...
            for(int x = 0; x < maze_width; x++) {
                std::cout << "+";
                if(maze[y][x].hasWall(NORTH)) std::cout << "---";
                else std::cout << "   ";
            }
            std::cout << "+" << std::endl;
            ...
            for(int x = 0; x < maze_width; x++) {
                if(maze[y][x].hasWall(WEST)) std::cout << "|";
                else std::cout << " ";
                
                // Print cell content
                if(Position(x, y) == current_pos) std::cout << " R ";
                else if(Position(x, y) == target_pos) std::cout << " T ";
                else if(maze[y][x].visited) std::cout << " . ";
                else std::cout << "   ";
            }
            if(maze_width > 0 && maze[y][maze_width-1].hasWall(EAST)) std::cout << "|";
            else std::cout << " ";
            std::cout << std::endl;
        }
        ...
        for(int x = 0; x < maze_width; x++) {
            std::cout << "+";
            if(x < maze_width && maze[0][x].hasWall(SOUTH)) std::cout << "---";
            else std::cout << "   ";
        }
        std::cout << "+" << std::endl;
    }
